...
inline void free_file(file_buffer_t *buf) {
    if (buf && buf->data) {
        if (buf->is_mmap) {
...
inline void sec_push(section_list_t *list, search_section_t sec) {
...
inline void sec_free(section_list_t *list) {
    if (list->items) {
        free(list->items);
    }
    list->items = NULL;
...
inline uint32_t xor_subtract_u32(uint32_t value, uint32_t sub, uint32_t key) {
...
inline bool deobf_meta(file_buffer_t *meta) {
    ...
    // Cho phép version >= 29 thay vì hardcode == 29 để hỗ trợ mùa mới
...
inline bool parse_elf(const file_buffer_t *buf, section_list_t *exec_secs, section_list_t *data_secs, uint64_t *out_image_base) {
    if (!buf || !buf->data || buf->size < sizeof(Elf64_Ehdr)) {
        return false;
    }
    
    const Elf64_Ehdr *ehdr = (const Elf64_Ehdr *)buf->data;
    
    // Kiểm tra ELF magic: 0x7F "ELF"
    ...
    // Kiểm tra ELF class (64-bit)
    if (ehdr->e_ident[4] != 2) { // ELFCLASS64
        return false;
    }
    
    // Kiểm tra endianness (little endian)
    if (ehdr->e_ident[5] != 1) { // ELFDATA2LSB
        return false;
    }
    
    // Kiểm tra program header
...
static int dl_iterate_find_libil2cpp(struct dl_phdr_info *info, size_t size, void *data) {
    ...
    return ctx->found ? 1 : 0; // dừng nếu đã tìm thấy
}

static inline bool load_libil2cpp_from_memory(file_buffer_t *unity_buf,
                                       section_list_t *exec_secs,
                                       section_list_t *data_secs,
    ...
    unity_buf->size = (size_t)ctx.max_end_vaddr; // cho phép scan theo off tương đối
    unity_buf->is_mmap = false; // KHÔNG munmap/free

    if (out_image_base_runtime) {
        // image_base theo runtime = base + min_vaddr (tương đương "lowest mapped addr")
        *out_image_base_runtime = (ctx.min_vaddr == UINT64_MAX) ? (uint64_t)ctx.base : (uint64_t)ctx.base + ctx.min_vaddr;
    }
    return true;
}

inline bool va_to_off(const section_list_t *secs, uint64_t va, uint64_t *out_off) {
...
inline bool va_to_off_any(const section_list_t *data_secs, const section_list_t *exec_secs, uint64_t va, uint64_t *out_off) {
...
inline bool off_to_va(const section_list_t *secs, uint64_t off, uint64_t *out_va) {
    for (size_t i = 0; i < secs->count; i++) {
        const search_section_t *s = &secs->items[i];
...
inline bool va_in(const section_list_t *secs, uint64_t va) {
    for (size_t i = 0; i < secs->count; i++) {
        const search_section_t *s = &secs->items[i];
        if (va >= s->address && va < s->addressEnd) {
            return true;
...
// Forward declaration để dùng trong các helper phía trên
inline const char *metadata_string(const file_buffer_t *meta, const uint32_t *hdr, uint32_t index);

inline bool find_bytes(const uint8_t *data, size_t data_len, const uint8_t *pat, size_t pat_len, size_t *out_pos) {
...
inline size_t find_refs(const file_buffer_t *buf, const section_list_t *data_secs, uint64_t target_va, uint64_t *out_refs, size_t max_refs) {
    size_t count = 0;
...
// Helper: lấy danh sách tên image từ metadata để đối chiếu
static inline int collect_image_names(const file_buffer_t *meta,
                               const uint32_t *hdr,
...
inline bool find_code_reg(const file_buffer_t *unity_buf,
    ...
    // Fallback: brute scan data sections tìm Il2CppCodeRegistration
...
inline bool find_meta_reg(const file_buffer_t *unity_buf,
                                const section_list_t *exec_secs,
                                const section_list_t *data_secs,
...
inline bool find_method_tok(const file_buffer_t *meta,
                       const uint32_t *hdr,
        ...
        for (int t = type_start; t < type_end; t++) {
            ...
            for (int m = m_start; m < m_end; m++) {
                ...
                if (mn && strcmp(mn, method_name) == 0) {
                    // Check parameter count nếu argsCount >= 0
...
inline bool find_field_idx(const file_buffer_t *meta,
                      const uint32_t *hdr,
                      const char *image_name,
                      const char *type_namespace,
                      const char *type_name,
...
inline bool get_method_ptr(const file_buffer_t *unity_buf,
                                         const section_list_t *data_secs,
                                         const section_list_t *exec_secs,
                                         uint64_t code_reg_va,
                                         const char *image_name,
...
inline bool get_field_off(const file_buffer_t *unity_buf,
                       const section_list_t *data_secs,
                       const section_list_t *exec_secs,
...
inline char *find_metadata_path() {
    ...
    {
        const char *known_meta =
            "/storage/emulated/0/Android/data/com.garena.game.kgtw/files/il2cpp/Metadata/global-metadata.dat";
        if (access(known_meta, R_OK) == 0) {
            strncpy(cached_path, known_meta, sizeof(cached_path) - 1);
            return strdup(cached_path);
        }
    }
...
inline bool init_cache(unity_cache_t *cache) {
    if (!cache) {
        return false;
    }
    memset(cache, 0, sizeof(*cache));
    
    char *unity_path = find_libil2cpp_path();
...
inline void free_cache(unity_cache_t *cache) {
    if (!cache) {
        return;
    }
...
static void init_global_cache_once() {
    g_unity_cache = (unity_cache_t *)malloc(sizeof(unity_cache_t));
    if (g_unity_cache) {
...
inline unity_cache_t *get_cached_unity() {
    pthread_once(&g_cache_init_once, init_global_cache_once);
    return g_unity_cache;
}

inline uint64_t FindMethodOffset(const char *image_name,
                          const char *type_namespace,
                          const char *type_name,
                          const char *method_name,
                          int argsCount) {
...
inline int FindFieldOffset(const char *image_name,
                    const char *type_namespace,
                    const char *type_name,
                    const char *field_name) {
    unity_cache_t *cache = get_cached_unity();
